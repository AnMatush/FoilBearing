    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int k = 0; k < num_foil; k++)
                ...
                int z = num_foil;             //число лепестков

                double alfa = 0.8;            //Коэффициент релаксации
                int Nit = 1;                //Число итераций прогона
                int Nit1 = 500;                     //число итераций расчёта давлений
                ...
                double[,,] PPXZ_f = new double[ny, nz, z];

                for (int it = 1; it <= Nit; it++)
                {
                    PPXZ = ReynoldsUstanovlenie(dx, DZ, lamb, dtau, SIGMA, alfa, Nit1, ny, nz, H, PXZ,k);
                }
              }
              

          
            for (int i = 0; i < num_y_of_foil; i++) { Console.WriteLine(Pressure[i, 20,1]); }
           ...
           /*
             ...
             double[,] Pressure = new double[max_iter_y, max_iter_z];
             for (int k = 0; k < 2 ; k++)
             {
               ...
               for (int i = num_i_of_zazor[k]; i < num_i_of_zazor[k+1]; i++)
                 {
                    if (j== max_num_foil) { j=0; j++; }
                 Massiv_Pressure[i, 0] = All_coord_number[i, 0];
                 Massiv_Pressure[i, 1] = Pressure[j / max_iter_z, j - (j / max_iter_z) * max_iter_z];
                  }
                
             }
            for (int i = num_i_of_zazor[0]; i < num_i_of_zazor[1]; i++) { Console.WriteLine(Massiv_Pressure[i, 1]); }
           /*
            //////////////////////////////////////////////////////////////
        ...
        //////////////////Подпрограмма 1//////////////////////////
        public static double[,,] ReynoldsUstanovlenie(double dx, double DZ, double lamb, double dtau, double SIGMA, double alfa, double Nit1, int ny, int nz, double[,,] H, double[,,] PXZ,int k)
        {
           //Переменные
            double u1;
            double u2;
            ...
            //Объявление зазоров
            double[,] H3p = new double[ny, nz,k];
            double[,] H3m = new double[ny, nz,k];
