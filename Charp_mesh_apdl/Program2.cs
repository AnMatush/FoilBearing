    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < num_y_of_foil; i++)
            {
                for (int j = 0; j < max_iter_z; j++)
                {
                    if ((max_iter_z * i + j) + num_i_of_zazor[num_foil - 1] < Num_strok) { H[i, j, num_foil - 1] = All_coord_number[(max_iter_z * i + j) + num_i_of_zazor[num_foil - 1], 6]; }
                    else { H[i, j, num_foil - 1]= All_coord_number[max_iter_z * i + j, 6]; }
                }
            }
